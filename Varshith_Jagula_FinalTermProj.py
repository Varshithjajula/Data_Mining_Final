...
def calculate_metrics(y_true, y_pred):
    ...
    Accuracy = (TP + TN) / (TP + TN + FP + FN)  # Custom Accuracy
...
for fold, (train_index, val_index) in enumerate(skf.split(X_train, Y_train), start=1):
    ...
    # Update metrics with AUC and Brier Score
    metrics['AUC'] = roc_auc_score(y_val_fold, y_val_prob)
    metrics['Brier_Score'] = brier_score_loss(y_val_fold, y_val_prob)
...
Y_train_reshaped = Y_train.values.astype(np.int32)  # Ensure Y_train is in integer format for classification
